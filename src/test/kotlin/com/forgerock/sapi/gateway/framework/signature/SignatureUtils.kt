package com.forgerock.sapi.gateway.framework.signature
...
fun validateCriticalParameters(headers: JWSHeader, version: OBVersion, tpp: Tpp) {
    ...
    if (headers.algorithm.name != "PS256")
        throw AssertionError("${com.forgerock.sapi.gateway.ob.uk.framework.errors.INVALID_DETACHED_JWS_ERROR} Invalid algorithm was used: ${headers.algorithm.name}")

    // typ - Optional header
    if (headers.type != null && headers.type.type != "JOSE")
        throw AssertionError("${com.forgerock.sapi.gateway.ob.uk.framework.errors.INVALID_DETACHED_JWS_ERROR} Invalid type detected: ${headers.type}")
    ...
    if (!(headers.customParams["http://openbanking.org.uk/iat"] != null && BigDecimal(headers.customParams["http://openbanking.org.uk/iat"] as Double) < BigDecimal(
        ...
        throw AssertionError("${com.forgerock.sapi.gateway.ob.uk.framework.errors.INVALID_DETACHED_JWS_ERROR} Invalid issued at timestamp - value from JWT: ${headers.customParams["http://openbanking.org.uk/iat"]} and current timestamp: $currentTimestamp")

    // http://openbanking.org.uk/tan
    if (!(headers.customParams["http://openbanking.org.uk/tan"] != null && headers.customParams["http://openbanking.org.uk/tan"] == com.forgerock.sapi.gateway.ob.uk.framework.constants.TAN))
        throw AssertionError("${com.forgerock.sapi.gateway.ob.uk.framework.errors.INVALID_DETACHED_JWS_ERROR} Invalid trusted anchor found: ${headers.customParams["http://openbanking.org.uk/tan"]} expected: ${com.forgerock.sapi.gateway.ob.uk.framework.constants.TAN}")
    ...
    ) throw AssertionError("${com.forgerock.sapi.gateway.ob.uk.framework.errors.INVALID_DETACHED_JWS_ERROR} Comparison of subject dns failed")

    // b64
    if (version.isBeforeVersion(OBVersion.v3_1_4) && headers.isBase64URLEncodePayload)
        throw AssertionError("${com.forgerock.sapi.gateway.ob.uk.framework.errors.INVALID_DETACHED_JWS_ERROR} B64 header must be false in JWT header before v3.1.4")
    if (!version.isBeforeVersion(OBVersion.v3_1_4) && !headers.isBase64URLEncodePayload)
        throw AssertionError("${com.forgerock.sapi.gateway.ob.uk.framework.errors.INVALID_DETACHED_JWS_ERROR} B64 header not permitted in JWT header after v3.1.3")
}
