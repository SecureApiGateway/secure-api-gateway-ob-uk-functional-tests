...
import com.forgerock.securebanking.support.event.EventsDataFactory.Companion.anEventDataRequest
...
class Events {
    ...
    fun importEvents(
        ...
        return createEventData(tpp, clientCredentials, version)
    }
    ...
    private inline fun <reified T : Any> createEventData(
        tpp: Tpp, accessToken: AccessToken, version: OBVersion =
            OBVersion.v3_1_2
    ): T {
    ...
    fun clientCredentialsAuthentication(tpp: Tpp, scopes: String): AccessToken {
        val clientCredentialsForm = listOf("grant_type" to "client_credentials", "scope" to scopes)
        val (_, response, result) = Fuel.post(asDiscovery.token_endpoint, parameters = clientCredentialsForm)
            .authentication()
            .basic(tpp.registrationResponse.client_id, tpp.registrationResponse.client_secret!!)
