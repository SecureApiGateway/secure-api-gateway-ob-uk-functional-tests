package com.forgerock.openbanking.servicecheck
...
class ServiceHealthCheckTest {
    ...
    fun getDirectoryState() {
        // When
        val (_, response, result) = Fuel.get("https://matls.service.directory.$DOMAIN/external/actuator/health").responseString()
        // Then
        assertThat(response.statusCode).isEqualTo(200)
        assertThat(result.get()).isNotNull()
        assertThat(result.get()).contains("UP")
    ...
    fun getBankState() {
        // When
        val (_, response, result) = Fuel.get("https://matls.service.bank.$DOMAIN/external/actuator/health").responseString()
        // Then
        assertThat(response.statusCode).isEqualTo(200)
        assertThat(result.get()).isNotNull()
        assertThat(result.get()).contains("UP")
