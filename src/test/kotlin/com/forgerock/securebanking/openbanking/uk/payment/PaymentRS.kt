package com.forgerock.openbanking.payment
...
class PaymentRS {
    ...
    companion object {
        ...
        private fun getJwkSet(): JWKSet {
            val (_, response, result) = Fuel.get("https://as.aspsp.$DOMAIN/api/jwk/jwk_uri").responseString()
            return if (response.isSuccessful) JWKSet.parse(result.get()) else throw AssertionError("Failed to retrieve JWKs", result.component2())
        }
    }

    inline fun <reified T : Any> consent(consentUrl: String, consentRequest: Any, tpp: Tpp, version: OBVersion = v3_1_2): T {
        try {
    ...
    fun submitFilePayment(consentFileUrl: String, file: String, mediaType: String, tpp: Tpp, version: OBVersion = v3_1_2): Response {
        try {
            val accessToken = getAccessToken(tpp)
    ...
    fun submitCSVFilePayment(consentFileUrl: String, file: String, mediaType: String, tpp: Tpp, version: OBVersion = v3_1_2): Response {
        try {
            val accessToken = getAccessToken(tpp)
            val detachedJwt = getDetachedJws(file, tpp, version)
            val (_, consentResponse, r) = Fuel.post(consentFileUrl)
    ...
    inline fun <reified T : Any> getConsent(consentUrl: String, tpp: Tpp): T {
        ...
        val (_, consentResponse, r) = Fuel.get(consentUrl)
                .defaultHeaders(accessToken)
                .responseObject<T>()
        if (!consentResponse.isSuccessful) throw AssertionError("Could not get consent with with: ${String(consentResponse.data)}", r.component2())
        return r.get()
    }

    inline fun <reified T : Any> submitPayment(paymentUrl: String, paymentRequest: Any, accessToken: AccessToken, tpp: Tpp, version: OBVersion = v3_1_2): T {
        val (_, response, r) = Fuel.post(paymentUrl)
                ...
                .responseObject<T>()
        if (!response.isSuccessful) throw AssertionError("Could not create payment submission with: ${String(response.data)}", r.component2())
        return r.get()
    }

    inline fun <reified T : Any> getPayment(url: String, paymentId: String, accessToken: AccessToken, version: OBVersion = v3_1_2): T {
        val (_, response, result) = Fuel.get("$url/$paymentId")
                .header("Authorization", "Bearer ${accessToken.access_token}")
                .header("x-fapi-financial-id", rsDiscovery.Data.FinancialId)
                .responseString()
        if (!response.isSuccessful) throw AssertionError("Could not get payment submission with: ${String(response.data)}", result.component2())
    ...
    inline fun <reified T : Any> consentRequest_InvalidDetachedJws(consentUrl: String, consentRequest: Any, tpp: Tpp): Response {
        val accessToken = getAccessToken(tpp)
    ...
    inline fun <reified T : Any> consentRequest_DetachedJwsMissingB64Claim(consentUrl: String, consentRequest: Any, tpp: Tpp): Response {
        val accessToken = getAccessToken(tpp)
        val detachedJwt = getDetachedJws(consentRequest, tpp, v3_1_4) // v3.1.4 onwards should not contain the b64 header
        val (_, consentResponse, _) = Fuel.post(consentUrl)
                .jsonBody(consentRequest)
                .defaultHeaders(accessToken)
                .header("x-jws-signature", detachedJwt)
    ...
    inline fun <reified T : Any> submitPayment_InvalidDetachedJws(paymentUrl: String, paymentRequest: Any, accessToken: AccessToken): Response {
    ...
    fun getAccessToken(tpp: Tpp): String {
        val scopes = asDiscovery.scopes_supported.intersect(listOf("accounts", "payments")).joinToString(separator = " ")
        val body = listOf(
    ...
    fun getDetachedJws(body: Any, tpp: Tpp, version: OBVersion): String {
        ...
        if (!detachedJwtResponse.isSuccessful) throw AssertionError("Could not get detached JWS", detachedJwt.component2())
