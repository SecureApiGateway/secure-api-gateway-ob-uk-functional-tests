...
class GetTransactionsTest(val tppResource: CreateTppCallback.TppResource) {
    ...
    @EnabledIfOpenBankingVersion(type = "accounts", version = "v3.1.1", apis = ["transactions"])
    @Test
    fun shouldGetAccountTransactions_v3_1_1() {
        // Given
        val consentRequest = OBReadConsent1().data(OBReadData1()
                .permissions(listOf(READACCOUNTSDETAIL, READTRANSACTIONSCREDITS, READTRANSACTIONSDEBITS, READTRANSACTIONSDETAIL)))
                .risk(OBRisk2())
        ...
        // When
        val result = AccountRS().getAccountData<OBReadTransaction5>(urlWithAccountId(accountAndTransaction3_1_1.Links.links.GetAccountTransactions, accountId), accessToken)
    ...
    @EnabledIfOpenBankingVersion(type = "accounts", version = "v3.1.6", apis = ["transactions"])
    @Test
    fun shouldGetAccountTransactions_v3_1_6() {
        ...
        // When
        val result = AccountRS().getAccountData<OBReadTransaction6>(urlWithAccountId(accountAndTransaction3_1_6.Links.links.GetAccountTransactions, accountId), accessToken)
