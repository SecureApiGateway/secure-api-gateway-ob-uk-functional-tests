package com.forgerock.openbanking.event
...
import com.forgerock.openbanking.event.EventsDataFactory.Companion.anEventDataRequest
...
class Events {
    ...
    fun importEvents(tpp: Tpp, clientCredentials: AccessToken, version: OBVersion): Collection<FREventNotification> {
    ...
    fun pollForEvents(tpp: Tpp, clientCredentials: AccessToken, version: OBVersion): OBEventPollingResponse1 {
    ...
    fun sendAcknowledgement(tpp: Tpp, clientCredentials: AccessToken, request: OBEventPolling1, version: OBVersion): OBEventPollingResponse1 {
        val url = getPollingUrl(version)
        return submitPost(url, request, clientCredentials, tpp, version)
    }

    private inline fun <reified T : Any> createEventData(tpp: Tpp, accessToken: AccessToken, version: OBVersion =
    OBVersion.v3_1_2): T {
        ...
        val (_, response, r) = Fuel.post("https://matls.service.bank.$DOMAIN/api/data/events")
                ...
                .responseObject<T>()
        if (!response.isSuccessful) throw AssertionError("Could not create event data URL with: ${String(response.data)}", r.component2())
        return r.get()
    }

    inline fun <reified T : Any> submitPost(eventNotificationUrl: String, eventRequest: Any, accessToken: AccessToken, tpp: Tpp, version: OBVersion = OBVersion.v3_1_2): T {
        val (_, response, r) = Fuel.post(eventNotificationUrl)
                ...
                .responseObject<T>()
        if (!response.isSuccessful) throw AssertionError("Could not create Callback URL with: ${String(response.data)}", r.component2())
        return r.get()
    }

    inline fun <reified T : Any> submitGet(eventNotificationUrl: String, accessToken: AccessToken): T {
        val (_, response, r) = Fuel.get(eventNotificationUrl)
                .defaultHeaders(accessToken.access_token)
                .responseObject<T>()
        if (!response.isSuccessful) throw AssertionError("Could not Get Callback URLS with: ${String(response.data)}", r.component2())
        return r.get()
    }

    inline fun <reified T : Any> submitPut(eventNotificationUrl: String, id: String, eventRequest: Any, accessToken: AccessToken, tpp: Tpp, version: OBVersion = OBVersion.v3_1_2): T {
        val (_, response, r) = Fuel.put("$eventNotificationUrl/$id")
                .jsonBody(eventRequest)
                .defaultHeaders(accessToken.access_token)
                .header("x-jws-signature", getDetachedJws(eventRequest, tpp, version))
                .responseObject<T>()
        if (!response.isSuccessful) throw AssertionError("Could not amend Callback URL with: ${String(response.data)}", r.component2())
        return r.get()
    }

    fun submitDelete(eventNotificationUrl: String, id: String, accessToken: AccessToken): Response {
        val (_, response, r) = Fuel.delete("$eventNotificationUrl/$id")
                .defaultHeaders(accessToken.access_token)
                .responseObject<Response>()
        if (!response.isSuccessful) throw AssertionError("Could not delete Callback URL with: ${String(response.data)}", r.component2())
        return response
    }

    fun clientCredentialsAuthentication(tpp: Tpp): AccessToken {
        val scopes = asDiscovery.scopes_supported.intersect(listOf("openid", "accounts", "payments")).joinToString(separator = " ")
