...
import com.forgerock.openbanking.directory.createSoftwareStatement
import com.forgerock.openbanking.directory.getSigningKid
import com.forgerock.openbanking.directory.getTransportKid
...
val serialisers = SimpleModule("CustomSerialiser")
...
val defaultMapper = ObjectMapper().registerKotlinModule()
        ...
        .registerModules(serialisers)

fun initFuel(privatePem: InputStream, publicPem: InputStream) {
    val ks = loadKeystore(privatePem, publicPem)
    val truststore = object {}.javaClass.getResource("truststore.jks")
    val sc = SSLContextBuilder()
            ...
            { _, _ -> "server" }
            .loadTrustMaterial(truststore.toURI().toURL(), "".toCharArray())
    initFuel(sc)
}

private fun initFuel(scb: SSLContextBuilder = SSLContextBuilder().loadTrustMaterial(object {}.javaClass.getResource("truststore.jks"), "".toCharArray())) {
 ...
 *
 * @param keystore defaults to open banking transport certs
 */
fun initFuel(privatePem: String = "/com/forgerock/openbanking/ob-eidas/obwac.key", publicPem: String = "/com/forgerock/openbanking/ob-eidas/obwac.pem") {
 ...
 */
inline fun <reified T : Any> Request.responseObject(): ResponseResultOf<T> = response(jacksonDeserializerOf(defaultMapper))
...
inline fun <reified T : Any> Request.jsonBody(src: String) = this.jsonBody(src)