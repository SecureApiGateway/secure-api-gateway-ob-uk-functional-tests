...
import com.forgerock.openbanking.account.AccountFactory.Companion.urlSubstituted
...
class SimpleAISPTest(val tppResource: CreateTppCallback.TppResource) {
    ...
    @EnabledIfOpenBankingVersion(type = "accounts", version = "v3.1.1", apis = ["accounts", "statements", "party"])
    @Test
    fun shouldBeSuccessfulForAllAisp_v3_1_1() {
        ...
        val statementId = AccountRS().getFirstStatementId(accountAndTransaction3_1_1.Links.links.GetStatements, accessToken)

        val apisV3_1_1 = rsDiscoveryMap["accounts"]?.filter { "v3.1.1".equals(it.first) }?.first()
        val apisExcludingFileAndConsent = apisV3_1_1?.second?.filter { !it.contains("account-access-consents") && !it.endsWith("/file") }
    ...
    @EnabledIfOpenBankingVersion(type = "accounts", version = "v3.1.4", apis = ["accounts", "statements", "party"])
    @Test
    fun shouldBeSuccessfulForAllAisp_v3_1_4() {
        ...
        val statementId = AccountRS().getFirstStatementId(accountAndTransaction3_1_4.Links.links.GetStatements, accessToken)

        val apisV3_1_4 = rsDiscoveryMap["accounts"]?.filter { "v3.1.4".equals(it.first) }?.first()
        val apisExcludingFileAndConsent = apisV3_1_4?.second?.filter { !it.contains("account-access-consents") && !it.endsWith("/file") }
        apisExcludingFileAndConsent?.forEach {
            // When
            val replacedUrl = urlSubstituted(it, mapOf("AccountId" to accountId, "StatementId" to statementId))
            val accountData = AccountRS().getAccountData<Any>(replacedUrl, accessToken)
    ...
    @EnabledIfOpenBankingVersion(type = "accounts", version = "v3.1.6", apis = ["accounts", "statements", "party"])
    @Test
    fun shouldBeSuccessfulForAllAisp_v3_1_6() {
        ...
        val statementId = AccountRS().getFirstStatementId(accountAndTransaction3_1_6.Links.links.GetStatements, accessToken)

        val apisV3_1_5 = rsDiscoveryMap["accounts"]?.filter { "v3.1.6".equals(it.first) }?.first()
        val apisExcludingFileAndConsent = apisV3_1_5?.second?.filter { !it.contains("account-access-consents") && !it.endsWith("/file") }
        apisExcludingFileAndConsent?.forEach {
            // When
            val replacedUrl = urlSubstituted(it, mapOf("AccountId" to accountId, "StatementId" to statementId))
            val accountData = AccountRS().getAccountData<Any>(replacedUrl, accessToken)
