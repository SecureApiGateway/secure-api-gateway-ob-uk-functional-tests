...
class CreateFilePaymentsConsents(val version: OBVersion, val tppResource: CreateTppCallback.TppResource) {
    ...
    fun createFilePaymentsConsentsTest() {
        // Given
        val fileContent = PaymentFactoryV4.getFileAsString(PaymentFactoryV4.FilePaths.XML_FILE_PATH)
        val consentRequest = PaymentFactoryV4.createOBWriteFileConsent3WithFileInfo(
            fileContent,
    ...
    fun submitFile_SameIdempotencyKeyMultipleRequestTest() {
        ...
        val response1 = paymentApiClient.newFilePostRequestBuilder(
            PaymentFactoryV4.urlWithFilePaymentSubmitFileId(paymentLinks.CreateFilePaymentFile, consent.data.consentId),
            tppResource.tpp.getClientCredentialsAccessToken(defaultPaymentScopesForAccessToken),
            fileContent,
    ...
    fun submitFile_NoIdempotencyKey_throwsBadRequestTest() {
        ...
        val exception = org.junit.jupiter.api.Assertions.assertThrows(AssertionError::class.java) {
            paymentApiClient.newFilePostRequestBuilder(
                PaymentFactoryV4.urlWithFilePaymentSubmitFileId(
    ...
    fun submitJSONFileTest() {
        // Given
        val fileContent = PaymentFactoryV4.getFileAsString(PaymentFactoryV4.FilePaths.JSON_FILE_PATH)
        ...
        val fileHash = PaymentFactoryV4.computeSHA256FullHash(fileContent)

        val consentRequest = PaymentFactoryV4.createJsonOBWriteFileConsent3WithFileInfo(
            fileHash,
            controlSum,
    ...
    fun createFilePaymentsConsents_mandatoryFields() {
        // Given
        val fileContent = PaymentFactoryV4.getFileAsString(PaymentFactoryV4.FilePaths.XML_FILE_PATH)
        val consentRequest = PaymentFactoryV4.createOBWriteFileConsent3WithMandatoryFieldsAndFileInfo(
            fileContent,
            PaymentFileType.UK_OBIE_PAIN_001_001_008.type
        )
    ...
    private fun buildSubmitFileRequest(
        ...
        PaymentFactoryV4.urlWithFilePaymentSubmitFileId(paymentLinks.CreateFilePaymentFile, consentId),
        tppResource.tpp.getClientCredentialsAccessToken(defaultPaymentScopesForAccessToken),
        fileContent,
        contentType
    )
